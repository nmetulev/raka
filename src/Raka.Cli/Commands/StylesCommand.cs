...
internal static class StylesCommand
{
    public static Command Create()
    {
        ...
        var command = new Command("styles", "Show the Style applied to an element (setters, target type, based-on)")
        {
            elementArg,
            nameOption
        };
        ...
        command.SetAction(async (parseResult) =>
        {
            ...
            var p = new StylesParams(element, name);
            var parameters = JsonSerializer.SerializeToElement(p, CliJsonContext.Default.StylesParams);
            Environment.ExitCode = await CommandHelpers.SendAndPrint(parseResult, Raka.Protocol.Commands.Styles, parameters);
        });
